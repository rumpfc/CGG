...
public:
	...
	// cofactor of matrix for row and col

	TransformMatrix3D cofactor(int row, int col);
	...
	// Build matrix from 3 vectors
	// needed for look at matrices

	void buildMatrixFromVectors(Vector3D& r0, Vector3D& r1, Vector3D& r2);
	...
	void operator=(const TransformMatrix3D& src);
};
