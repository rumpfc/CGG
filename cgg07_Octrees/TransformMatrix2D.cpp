...
void TransformMatrix2D::inverse()
{
	...
	for (int r = 0; r < 3; r++)
	{
		for (int c = 0; c < 3; c++)
		{
			TransformMatrix2D temp = this->cofactor(r, c);
			double det = temp.det();
...
void TransformMatrix2D::transpose()
{
...
TransformMatrix2D TransformMatrix2D::cofactor(int row, int col)
{
	TransformMatrix2D result(*this);
	for (int r = 0; r < 3; r++)
	{
		for (int c = 0; c < 3; c++)
		{
...
void TransformMatrix2D::operator=(const TransformMatrix2D& src)
{
	for (int r = 0; r < 3; r++)
	{
		for (int c = 0; c < 3; c++)
